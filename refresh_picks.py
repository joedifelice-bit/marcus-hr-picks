...
def model_p_hr(hitter, pitcher_stats, park_hr, expected_pa):
    """
    Simplified v2 model (without Statcast barrel% — falls back to season HR rate).
    P(HR per game) = base_hr_per_pa × park_factor × pitcher_HR9_mult × expected_pa
    """
    ...
    p_per_pa = base_hr_per_pa * park_hr * pitcher_mult
    # Probability of at least 1 HR in expected_pa attempts
...
def build_picks_for_game(game, season, fd_odds_map):
    ...
    for side in ("away", "home"):
        ...
        for h in hitters:
            ...
            # Estimate lineup spot by HR rank (top hitters bat 2-5 typically)
            expected_pa = 4.3 if h.get("hr_per_g", 0) >= 0.15 else 4.0
            p_hr = model_p_hr(h, opp_p_stats, park_hr, expected_pa)
